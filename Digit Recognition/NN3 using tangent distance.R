nn3 <- function(trainob, testob, trainlab){
          source("tangent.R")                     #load the function of tangent distance
          ...
          for(testrow in 1:nrow(testob)){         #go over all the test objects
            ...
            for(trainrow in 1:nrow(trainob)){     #go over all the training objects
              distance <- tangentdistance(trainob[trainrow,], testob[testrow,])    #calculate the tangent distance between test objects and training objects 
